...
function reviactyl_GetHostname(array $params) {
...
function reviactyl_API(array $params, $endpoint, array $data = [], $method = "GET", $dontLog = false) {
    $url = reviactyl_GetHostname($params) . '/api/application/' . $endpoint;
    ...
    curl_setopt($curl, CURLOPT_USERAGENT, "Reviactyl-WHMCS");
    ...
    $headers = [
        ...
        "Accept: Application/vnd.reviactyl.v26+json",
    ];
    ...
    if($responseData['status_code'] === 0 && !$dontLog) logModuleCall("Reviactyl-WHMCS", "CURL ERROR", curl_error($curl), "");

    curl_close($curl);

    if(!$dontLog) logModuleCall("Reviactyl-WHMCS", $method . " - " . $url,
...
function reviactyl_Error($func, $params, Exception $err) {
    logModuleCall("Reviactyl-WHMCS", $func, $params, $err->getMessage(), $err->getTraceAsString());
}

function reviactyl_MetaData() {
    return [
        "DisplayName" => "Reviactyl",
        "APIVersion" => "1.1",
        "RequiresServer" => true,
    ];
}

function reviactyl_ConfigOptions() {
    return [
        "cpu" => [
...
function reviactyl_TestConnection(array $params) {
    $solutions = [
    ...
    $err = "";
    try {
        $response = reviactyl_API($params, 'nodes');
        ...
        reviactyl_Error(__FUNCTION__, $params, $e);
        $err = $e->getMessage();
    }
...
function reviactyl_GenerateUsername($length = 8) {
    $returnable = false;
    while (!$returnable) {
...
function reviactyl_GetOption(array $params, $id, $default = NULL) {
    $options = reviactyl_ConfigOptions();
    ...
    $found = false;
    $i = 0;
    foreach(reviactyl_ConfigOptions() as $key => $value) {
        $i++;
        if($key === $id) {
            $found = true;
            break;
...
function reviactyl_CreateAccount(array $params) {
    try {
        $serverId = reviactyl_GetServerID($params);
        if(isset($serverId)) throw new Exception('Failed to create server because it is already created.');

        $userResult = reviactyl_API($params, 'users/external/' . $params['clientsdetails']['id']);
        if($userResult['status_code'] === 404) {
            $userResult = reviactyl_API($params, 'users?filter[email]=' . urlencode($params['clientsdetails']['email']));
            if($userResult['meta']['pagination']['total'] === 0) {
                $userResult = reviactyl_API($params, 'users', [
                    'username' => reviactyl_GetOption($params, 'username', reviactyl_GenerateUsername()),
        ...
        $nestId = reviactyl_GetOption($params, 'nest_id');
        $eggId = reviactyl_GetOption($params, 'egg_id');

        $eggData = reviactyl_API($params, 'nests/' . $nestId . '/eggs/' . $eggId . '?include=variables');
        ...
        foreach($eggData['attributes']['relationships']['variables']['data'] as $key => $val) {
            ...
            $friendlyName = reviactyl_GetOption($params, $attr['name']);
            $envName = reviactyl_GetOption($params, $attr['env_variable']);
        ...
        $name = reviactyl_GetOption($params, 'server_name', reviactyl_GenerateUsername() . '_' . $params['serviceid']);
        $memory = reviactyl_GetOption($params, 'memory');
        $swap = reviactyl_GetOption($params, 'swap');
        $io = reviactyl_GetOption($params, 'io');
        $cpu = reviactyl_GetOption($params, 'cpu');
        $disk = reviactyl_GetOption($params, 'disk');
        $location_id = reviactyl_GetOption($params, 'location_id');
        $dedicated_ip = reviactyl_GetOption($params, 'dedicated_ip') ? true : false;
        $port_range = reviactyl_GetOption($params, 'port_range');
        $port_range = isset($port_range) ? explode(',', $port_range) : [];
        $image = reviactyl_GetOption($params, 'image', $eggData['attributes']['docker_image']);
        $startup = reviactyl_GetOption($params, 'startup', $eggData['attributes']['startup']);
        $databases = reviactyl_GetOption($params, 'databases');
        $allocations = reviactyl_GetOption($params, 'allocations');
        $backups = reviactyl_GetOption($params, 'backups');
        $oom_disabled = reviactyl_GetOption($params, 'oom_disabled') ? true : false;
        $serverData = [
            'name' => $name,
            'user' => (int) $userId,
            'nest' => (int) $nestId,
        ...
        $server = reviactyl_API($params, 'servers?include=allocations', $serverData, 'POST');
...
function reviactyl_GetServerID(array $params, $raw = false) {
    $serverResult = reviactyl_API($params, 'servers/external/' . $params['serviceid'], [], 'GET', true);
    ...
    if(Capsule::schema()->hasTable('tbl_pterodactylproduct')) {
        ...
        if(isset($oldData) && isset($oldData->server_id)) {
            if($raw) {
                $serverResult = reviactyl_API($params, 'servers/' . $oldData->server_id);
...
function reviactyl_SuspendAccount(array $params) {
    try {
        $serverId = reviactyl_GetServerID($params);
        if(!isset($serverId)) throw new Exception('Failed to suspend server because it doesn\'t exist.');

        $suspendResult = reviactyl_API($params, 'servers/' . $serverId . '/suspend', [], 'POST');
...
function reviactyl_UnsuspendAccount(array $params) {
    try {
        $serverId = reviactyl_GetServerID($params);
        if(!isset($serverId)) throw new Exception('Failed to unsuspend server because it doesn\'t exist.');

        $suspendResult = reviactyl_API($params, 'servers/' . $serverId . '/unsuspend', [], 'POST');
...
function reviactyl_TerminateAccount(array $params) {
    try {
        $serverId = reviactyl_GetServerID($params);
        if(!isset($serverId)) throw new Exception('Failed to terminate server because it doesn\'t exist.');

        $deleteResult = reviactyl_API($params, 'servers/' . $serverId, [], 'DELETE');
...
function reviactyl_ChangePassword(array $params) {
    try {
        if($params['password'] === '') throw new Exception('The password cannot be empty.');

        $serverData = reviactyl_GetServerID($params, true);
...
function reviactyl_ChangePackage(array $params) {
    try {
        $serverData = reviactyl_GetServerID($params, true);
        ...
        $updateResult = reviactyl_API($params, 'servers/' . $serverId . '/build', $updateData, 'PATCH');
        ...
        $image = reviactyl_GetOption($params, 'image', $serverData['attributes']['container']['image']);
        $startup = reviactyl_GetOption($params, 'startup', $serverData['attributes']['container']['startup_command']);
        $updateData = [
            'environment' => $environment,
            'startup' => $startup,
            'egg' => (int) $eggId,
        ...
        $updateResult = reviactyl_API($params, 'servers/' . $serverId . '/startup', $updateData, 'PATCH');
...
function reviactyl_LoginLink(array $params) {
    if($params['moduletype'] !== 'reviactyl') return;

    try {
        $serverId = reviactyl_GetServerID($params);
        if(!isset($serverId)) return;

        $hostname = reviactyl_GetHostname($params);
        echo '<a style="padding-right:3px" href="'.$hostname.'/admin/servers/view/' . $serverId . '" target="_blank">[Go to Service]</a>';
        echo '<p style="float:right; padding-right:1.3%">[<a href="https://github.com/reviactyl/whmcs/issues" target="_blank">Report A Bug</a>]</p>';
        # echo '<p style="float: right">[<a href="https://github.com/reviactyl/whmcs/issues" target="_blank">Report A Bug</a>]</p>';
    } catch(Exception $err) {
        // Ignore
    }
}

function reviactyl_ClientArea(array $params) {
    if($params['moduletype'] !== 'reviactyl') return;
